...
def search_algo_vcs(n, maze, start, end):
    ...
    while l.getLength()>0:
        ...
        cost, u, path = l.delete()
        row = u//n
        col = u%n
        maze[row][col] = -1

        if row + 1<n and maze[row+1][col] == 0 and vis[row + 1][col] == 0:
            l.insert([cost + 3, get_pos(row+1, col, n), copy.deepcopy(path) + ['down']])
            vis[row + 1][col] = 1
            exploration_cost += 3
        if row-1>=0 and maze[row-1][col] == 0 and vis[row-1][col] == 0:
            l.insert([cost + 2, get_pos(row-1, col, n), copy.deepcopy(path) + ['up']])
            vis[row -1 ][col] = 1
            exploration_cost += 2
        if col-1>=0 and maze[row][col-1] == 0  and vis[row][col-1] == 0:
            l.insert([cost +2, get_pos(row, col-1, n), copy.deepcopy(path) + ['left']])
            vis[row][col - 1] = 1
            exploration_cost += 2
        if col+1<n and maze[row][col+1] == 0 and vis[row ][col + 1] == 0:
            l.insert([cost + 2, get_pos(row, col+1, n), copy.deepcopy(path) + ['right']])
            vis[row][col + 1] = 1
            exploration_cost += 2
