    ...
    def delete(self): 
        try: 
            ...
            for i in range(len(self.queue)): 
                if self.queue[i][0] < self.queue[mini][0]: 
                    mini = i 
...
def getManhattanDistance(pos, end, n):
    return abs(pos//n - end//n) + abs(pos%n - end%n)

def search_algo_astar(n, maze, start, end):
    ...
    l = PriorityQueue()
    path = []
    l.insert([getManhattanDistance(start, end, n),0, pos, path])
    vis = [[0 for i in range(n)] for j in range(n)]
    while l.getLength()>0:
        max_recursion_depth = max(max_recursion_depth, l.getLength())
        _, cost, u, path = l.delete()
        row = u//n
        col = u%n
        maze[row][col] = -1

        if row + 1<n and maze[row+1][col] == 0 and vis[row + 1][col] == 0:
            pos = get_pos(row+1, col, n)
            heur = getManhattanDistance(pos, end, n)
            l.insert([cost + 3 + heur, cost + 3, pos, copy.deepcopy(path) + ['down']])
            vis[row+1][col] = 1
            exploration_cost+=3
        if row-1>=0 and maze[row-1][col] == 0 and vis[row-1][col] == 0:
            pos = get_pos(row-1, col, n)
            heur = getManhattanDistance(pos, end, n)
            l.insert([cost + 2 + heur, cost + 2, pos, copy.deepcopy(path) + ['up']])
            vis[row-1][col] =1
            exploration_cost+=2
        if col-1>=0 and maze[row][col-1] == 0  and vis[row][col - 1]==0:
            pos = get_pos(row, col-1, n)
            heur = getManhattanDistance(pos, end, n)
            l.insert([cost + 2 + heur, cost + 2, pos, copy.deepcopy(path) + ['left']])
            vis[row][col-1] =1
            exploration_cost+=2
        if col+1<n and maze[row][col+1] == 0 and vis[row][col + 1]==0:
            pos = get_pos(row, col+1, n)
            heur = getManhattanDistance(pos, end, n)
            l.insert([cost + 2 + heur, cost + 2, pos, copy.deepcopy(path) + ['right']])
            vis[row][col+1] =1
            exploration_cost+=2
        ...
        if u==end:
            ...
            print("Memory(Max length of recursion stack): ", max_recursion_depth)
            return
...
if __name__ == "__main__":
    ...
    search_algo_astar(n, maze, start, end)
